const general = {
  ...
  actions: {
    ...
    checkServerConnection({ commit, state }) {
      axios
        .get('/api/connection?error=' + state.showServerErrorModal + '&platform=' + window.navigator.platform)
        .then(() => {
          if (state.showServerErrorModal) location.reload()
        })
        .catch(() => {
    ...
    moveOrdersToTable({ dispatch }, data) {
      axios.post(`/api/orders/move/${data.from}/${data.to}`).then(() => {
        dispatch('getTables')
      })
    },
    getWaiters({ commit }) {
    ...
    getTableOrders({ commit, state }) {
      axios.get('/api/orders/table/' + state.activeTable.id).then((res) => {
        commit('clearOrder')
        commit('setNewOrders', res.data.data)
      })
    },
    getCurrentTable({ commit, dispatch }, id) {
      axios.get('/api/table/' + id).then((res) => {
    ...
    storeOrder({ commit, dispatch, state }, table_id) {
      ...
      axios
        .post('/api/orders', { table_id, order: state.order, total })
        .then((res) => {
    ...
    cashOut({ commit, dispatch, state }, data) {
      ...
      axios
        ...
        .then((res) => {
          ...
          if (res.data.data.status) dispatch('setPrintingInvoice', res.data.data, { root: true })
          dispatch('getTables')
