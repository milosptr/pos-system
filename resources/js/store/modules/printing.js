    ...
    setEpsonDevice({ dispatch, commit, state }, reprint) {
      ...
      state.ePosDev.connect("192.168.200.80", 8008, (data) => {
        ...
        if (data == "OK" || data == "SSL_CONNECT_OK") {
          ...
          state.ePosDev.createDevice(
            "local_printer",
            state.ePosDev.DEVICE_TYPE_PRINTER,
            { crypto: false, buffer: false },
            (devobj, retcode) => {
              console.log("PRINTER_createDevice", devobj, retcode);
              if (retcode == "OK") {
                console.log("PRINTER_createDevice_OK");
                state.printer = devobj;
                state.printer.timeout = 60000;
                state.printer.oncoveropen = function () {
                  console.log("coveropen");
                };
                state.printer.onreceive = function (res) {
                  if (res.success) {
                    // successfull printing
                    commit("resetPrinting");
                  } else {
                    // printing error
                    console.log(
                      "REPRINTING_ERROR_ONRECEIVE"
                      );
                      Sentry.captureMessage("REPRINTING_ERROR_ONRECEIVE");
        ...
        setPrintingOrder({ dispatch, state }, order) {
            ...
            if (state.printer === null) {
                ...
                Sentry.captureMessage("PRINTER_NOT_SET (setPrintingOrder)");
                return;
            }
        ...
        setPrintingInvoice({ dispatch, state }, invoice) {
            ...
            if (state.printer === null) {
                dispatch("setEpsonDevice", true);
                console.error("PRINTER_NOT_SET");
                Sentry.captureMessage("PRINTER_NOT_SET (setPrintingInvoice)");
                return;
            }
            const orders = invoice.order.sort((a, b) =>
                a.timestamp ? a.timestamp - b.timestamp : 1
            );

            state.printer.addLayout(
                state.printer.LAYOUT_RECEIPT,
                800,
                0,
                -8,
                0,
            ...
            state.printer.addText(
                printerTextBetween("Reon:", invoice.location)
            );
            state.printer.addText(
                printerTextBetween(
                    "Sto broj:",
                    parseInt(invoice.table.table_number).toString()
                )
            );
            state.printer.addText(
                "   —————————————— PROMET - PRODAJA —————————————\n"
            ...
            orders.forEach((item) => {
                if (!item.refund) {
                    state.printer.addText(
                        "   " + item.name + " (Ђ)/" + item.unit + "\n"
                    );
                    state.printer.addText(
                        printerItemPriceText(
            ...
            if (invoice.discount) {
                ...
                state.printer.addText(
                    printerTextBetween(
                        "Popust " + parseInt(invoice.discount) + "%:",
            ...
            state.printer.addText(
                printerTextBetween("Ђ        О-ПДВ       20% ", invoice.tax)
            );
            state.printer.addText(
                printerTextBetween("Ukupan iznos poreza:", invoice.tax)
            );
            state.printer.addText(
                "   =============================================\n"
            );
...
const printerItemPriceText = (price, qty, total) => {
    ...
    return (
        " ".repeat(startSpaces) +
        price +
        " ".repeat(afterPriceSpaces) +
        qty +
        " ".repeat(afterQtySpaces) +
        total +
        "\n"
    );
};
