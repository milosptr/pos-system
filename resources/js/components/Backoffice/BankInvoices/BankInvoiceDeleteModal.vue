<template>
  <div class="fixed top-0 left-0 w-full h-screen flex items-center justify-center z-[100]">
    ...
    <div class="bg-white w-full h-screen md:h-auto md:w-1/2 rounded-md px-4 py-6 z-[101]">
      <div class="">
        <h3 class="text-2xl font-semibold leading-6 text-gray-900" id="modal-title">Brisanje fakture</h3>
        <div class="mt-6">
      ...
      <div class="mt-5 sm:mt-10 flex flex-col sm:flex-row justify-between gap-8">
        ...
        <div class="sm:w-1/2 text-center relative px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 cursor-pointer" @click="deleteInvoice">Izbriši</div>
      </div>
    </div>
  </div>
</template>
...
<script>
  export default {
    ...
    methods: {
      deleteInvoice() {
        axios.put(`/api/bank-invoices/${this.invoice.id}`, {status: 2})
          .then(() => {
