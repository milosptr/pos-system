  ...
  <div class="w-full">
    <div class="flex flex-col justify-end gap-4 sm:flex-row">
      ...
      <div v-if="!tabInvoices">
        ...
        <select
          name="category"
          class="block w-full rounded-md border-gray-300 pl-3 pr-10 text-base focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:w-48 sm:text-sm"
      ...
      <div v-if="tabInvoices">
        ...
        <select
          name="waiters"
          class="block w-full rounded-md border-gray-300 pl-3 pr-10 text-base focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm"
  ...
  return [
    {
      ...
      atClick: () => {
        const date = new Date()
        return [new Date(date.setDate(date.getDate())), date]
      }
    },
    {
      label: 'Tomorrow',
...
export default {
  ...
  mounted() {
    ...
    axios.get('/api/waiters').then((res) => {
  ...
  methods: {
    updateReportFilters(key, value) {
      ...
      if (this.type === 'reports') this.$store.dispatch('getReports')
      if (this.type === 'invoices') this.$store.dispatch('getInvoices')
    },
