    ...
    <div class="flex items-center justify-end gap-5">
      <div>
        <label
          ...
          >Šank ili kuhinja</label
        >
        <Select
          :preselected="occupation"
          :list="occupations"
            ...
            <tbody class="bg-white">
              ...
              <tr>
                <th
                  ...
                  :key="idx"
                  scope="col"
                  class="sticky bottom-5 z-10 border-t border-gray-300 bg-gray-50 bg-opacity-75 py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-gray-900 backdrop-blur backdrop-filter sm:pl-6 lg:pl-8">
              ...
              <tr>
                <th
                  colspan="10"
                  class="sticky bottom-0 z-[9] h-5 w-full bg-gray-100"></th>
              </tr>
            </tbody>
          </table>
        </div>
    ...
    printTotal(key) {
      ...
      try {
        const checkins = this.arrivals?.data
          ...
          : []
        if (checkins.length) {
          const fullDays = checkins.filter((value, index, self) => {
            return self.findIndex((v) => v.created_date === value.created_date) === index
          })
          const totalInMinutes = checkins.reduce(
            (accumulator, currentValue) => this.parseMinutes(currentValue.total) + accumulator,
            0
          )
