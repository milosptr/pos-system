  ...
  <Modal :superWide="true" bodyClass="overflow-scroll">
        ...
        <tbody>
          <tr v-for="(task, idx) in tasks" :key="idx" class="hover:bg-orange-50 cursor-pointer text-lg" :class="{'bg-gray-50': idx % 2 === 1, 'text-green-600': task.done }">
            ...
            <td class="w-32 border-b border-gray-200 whitespace-nowrap py-2 px-3 font-medium text-right">
              <input :id="`task-${tasks.id}`" :name="`task-${tasks.id}`" type="checkbox" :checked="task.done" class="focus:ring-green-500 h-5 w-5 text-green-600 border-2 border-red-500 rounded outline-none ring-0 focus:ring-0" @click="finishTask(task)" />
            </td>
          </tr>
        </tbody>
     </table>
     ...
     <div class="flex flex-col gap-2">
      <div v-for="(message, idx) in messages" :key="idx" class="text-lg rounded-md bg-gray-100 p-4 flex items-start justify-between">
        ...
        <div class="">
          <input :id="`task-${message.id}`" :name="`task-${message.id}`" type="checkbox" :checked="message.done" class="focus:ring-green-500 h-5 w-5 text-green-600 border-2 border-red-500 rounded outline-none ring-0 focus:ring-0" @click="finishTask(message)" />
        </div>
      </div>
     </div>
  </Modal>
